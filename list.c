...
static Node* releasedNodes = NULL;  // Pointer to keep track of freed nodes (node stack)
static List* releasedHeads = NULL;  // Pointer to keep track of freed lists (head stack)

// enum needed to simplify stack functions for which stack to push to
// no need to decalare as static since I don't define a variable
// and only variables can be static not types
// see
// https://stackoverflow.com/questions/2743949/useless-class-storage-specifier-in-empty-declaration?rq=1
enum type
{
    LIST,
    NODE
...
static void* getAvailableNode();  // method to fetch available node
static void* getAvailableList();  // method to fetch available list
