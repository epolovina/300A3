...
void* keyboard(void* unused) {
  ...
  while (1) {
    ...
    if (readStdin > 0) {
      terminateIdx =
          (strlen(input) < MSG_MAX_LEN) ? (strlen(input)) : MSG_MAX_LEN - 1;
  ...
  while (1) {
    ...
    pthread_mutex_lock(&readInputListMutex);
    {
      ...
      for (int i = 0; i < List_count(pList_input); i++) {
        ...
        send = sendto(sockFD, ret, strlen(ret), 0, (struct sockaddr*)&sin,
                      sin_len);
        if (send == -1) {
...
static void listFreeFn(void* pItem) { pItem = NULL; }
